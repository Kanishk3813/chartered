              ...
              <div className="mt-8 grid gap-6 md:grid-cols-2">
                ...
                <div className="bg-white p-6 rounded-xl shadow-md">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Link href="/loan-application" className="flex flex-col items-center p-4 border border-gray-200 rounded-lg hover:bg-blue-50 hover:border-blue-200 transition-colors duration-300">
                    ...
                    <Link href="/documents" className="flex flex-col items-center p-4 border border-gray-200 rounded-lg hover:bg-blue-50 hover:border-blue-200 transition-colors duration-300">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-blue-600 mb-2" viewBox="0 0 20 20" fill="currentColor">
                        <path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clipRule="evenodd" />
                      </svg>
                      <span className="text-sm font-medium text-gray-700">Upload Documents</span>
                    </Link>
                    <Link href="/profile" className="flex flex-col items-center p-4 border border-gray-200 rounded-lg hover:bg-blue-50 hover:border-blue-200 transition-colors duration-300">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-blue-600 mb-2" viewBox="0 0 20 20" fill="currentColor">
                        <path fillRule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clipRule="evenodd" />
                      </svg>
                      <span className="text-sm font-medium text-gray-700">Update Profile</span>
                    </Link>
                    <Link href="/support" className="flex flex-col items-center p-4 border border-gray-200 rounded-lg hover:bg-blue-50 hover:border-blue-200 transition-colors duration-300">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-blue-600 mb-2" viewBox="0 0 20 20" fill="currentColor">
...
<span className="text-sm font-medium text-gray-700">Contact Support</span>
              ...
              <div className="mt-8 bg-white p-6 rounded-xl shadow-md">
                <h3 className="text-lg font-semibold mb-4">Loan Eligibility Status</h3>
                <div className="space-y-4">
                  {applications.map((application) => (
                    <div key={`status-${application.id}`} className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-gray-200 pb-4 last:border-0 last:pb-0">
                      <div>
                        <p className="font-medium text-gray-900">{application.type.charAt(0).toUpperCase() + application.type.slice(1)} Loan</p>
                      ...
                      <div className="mt-2 sm:mt-0 flex items-center">
                        <div className={`h-2 w-24 rounded-full ${
                          ...
                          application.status === 'in_review' ? 'bg-yellow-500' : 'bg-blue-500'
                        }`}></div>
                        <span className="ml-3 text-sm font-medium text-gray-700">
                          {application.status === 'approved' ? '100%' : 
                           application.status === 'rejected' ? '0%' : 
                           application.status === 'in_review' ? '75%' : '25%'}
                        </span>
                      </div>
                    </div>
                  ))}
